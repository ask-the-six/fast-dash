...
#%%
KMDA_T1=Targets['KMDA'] 
X = pd.concat([Continuous, Discrete,NominalDummy,OrdinalDummy,FeatureTargets,KMDA_T1], axis= 1)
#X = X.drop(X[X.STRA > 20000 ].index)
X = X.drop(X[X.KMDA == -9999].index)
X = X.drop(X[X.KMDA == 0].index)
KMDA_T1 = X['KMDA']
X = X.drop('KMDA',1)
X=X.apply(pd.to_numeric)
#KStart = KStart.drop(KStart[KStart.ATPEYR1 <1900 ].index)
KMDA_Features= list(X.columns.values)



#%%
labels = np.array(KMDA_T1)
...
KMDA_rf = RandomForestRegressor(n_estimators = 1000, random_state = 100)
KMDA_rf.fit(train_features, train_labels);
#%%
test_labels = test_labels.astype(np.float)
predictions = KMDA_rf.predict(test_features)
errors = abs(predictions - test_labels)
print('Mean Error:', round(np.mean(errors), 2), 'KM')


 #%%Categorical Results
from sklearn import metrics
importances = list(KMDA_rf.feature_importances_)
